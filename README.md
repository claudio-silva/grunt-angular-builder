...
```js
/*! restricted_javascript_code */
```

The javascript inside the special comment block `/*! */` is executed in an isolated scope, where some specific javascript functions are available. All variables or functions defined in the block are discarded after execution.

The exposed API contains the following functions:

function  | parameters | description
----------|------------|------------
`define`  | var, value | Assigns a value to a build variable.
`unset`   | var        | Clears the value of a build variable.
`require` | filename   | Forces inclusion of the specified script or stylesheet.

Of course, you can use logical constructs in your directives (`if`, `else`, etc.).

#### Requiring stylesheets

To declare a dependency between your module and one or more CSS stylesheets, include one (or more) `require` build-directives into one (or more) of the javascript source files that comprise each module.

Requiring a stylesheet:

```js
/*! require ("stylesheet/path/and/filename.css") */
```

Whenever a module is included in a build, all `require` build-directives present in any of the files that comprise the module are processed and the required stylesheets are included in the build.

This means that each individual source file may specify its style requirements.

> Duplicate require directives are ignored, so each stylesheet is included only once.

The specified path should be a relative path (not starting with `/`), which will be relative to the file containing the directive.
...
However, if an `assetsTargetFolder` is specified for the build target, assets are stored in subfolders of the specified location and urls in the final stylesheet are rebased to that location.
